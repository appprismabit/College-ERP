...
<body class="hold-transition sidebar-mini sidebar-collapse">
                            ...
                            <div class="card text-sm">
                                <div class="card-header">
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <div class="col-3">
                                            <label>&nbsp;</label>
                                            <button type="button" class="btn btn-success form-control" onclick="showTechnicianReportByDate()">
                                                Show Details
                                            </button>
                                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            fetchTechnician(() => {
                $('#technician').val('all'); // Ensure "all" is selected
                showTechnicianReportByDate();
            });
        ...
        function fetchTechnician(callback) {
            $.ajax({
                url: "<?php echo BASE_URL; ?>/master/getAllTechnician",
                type: "POST",
                success: function(response) {
                    try {
                        ...
                        $.each(response.data, function(index, data) {
                            select.append('<option value="' + data.user_id + '">' + data.user_title + '</option>');
                        });
        ...
        function showTechnicianReportByDate() {
            const from_Date = $('#fromDate').val();
            const to_Date = $('#toDate').val();
            const technicianId = $('#technician').val();
            const technicianName = $('#technician option:selected').text();

            if (!from_Date || !to_Date || !technicianId) {
                swal({
                    title: "Empty Fields",
            ...
            if (technicianName === "All") {
                $('#fromDateAndTodateText').text("All technician Report From " + formatToDMY(from_Date) + " to " + formatToDMY(to_Date));

            } else {
                $('#fromDateAndTodateText').text(technicianName + " Report From " + formatToDMY(from_Date) + " to " + formatToDMY(to_Date));
            ...
            $.ajax({
                url: "<?php echo BASE_URL ?>/master/showDetails",
                type: "POST",
                data: {
                    fromDate: fromDate,
                    toDate: toDate,
        ...
        function all_complaints_single(complaint_id) {
            var form = $('<form>', {
                'action': '<?php echo BASE_URL; ?>/router/technicianWiseReport',
                'method': 'POST',
            }).append($('<input>', {
                'type': 'hidden',
