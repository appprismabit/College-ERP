...
<body class="hold-transition sidebar-mini sidebar-collapse">
                            ...
                            <div class="card text-sm">
                                <div class="card-header">
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <div class="col-3">
                                            <label>&nbsp;</label>
                                            <button type="button" class="btn btn-success form-control" onclick="showWardNoReportByDate()">
                                                Show Details
                                            </button>
                                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            fetchWardNo(() => {
                $('#wardNo').val('1'); // Ensure "all" is selected
                showWardNoReportByDate();
            });
        ...
        function fetchWardNo(callback) {
            $.ajax({
                url: "<?php echo BASE_URL; ?>/master/getAllWardNo",
                type: "POST",
                success: function(response) {
                    try {
                        ...
                        var select = $('#wardNo');
                        select.empty();
                        select.append('<option value="">-- SELECT WARD NO  --</option>');


                        $.each(response.data, function(index, data) {
                            select.append('<option value="' + data.sourcewardcode + '">' + data.ward_lgd_name + '</option>');
                        });
        ...
        function showWardNoReportByDate() {
            const from_Date = $('#fromDate').val();
            const to_Date = $('#toDate').val();
            const wardNo = $('#wardNo').val();
            const wardName = $('#wardNo option:selected').text();

            if (!from_Date || !to_Date || !wardNo) {
                swal({
                    title: "Empty Fields",
            ...
            $('#fromDateAndTodateText').text(wardName + " Report From " + formatToDMY(from_Date) + " to " + formatToDMY(to_Date));
            ...
            $.ajax({
                url: "<?php echo BASE_URL ?>/master/showDetailsWardNoReport",
                type: "POST",
                data: {
                    fromDate: fromDate,
                    toDate: toDate,
